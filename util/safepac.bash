...
while getopts ":dln:W:I:hyu" option
do
	case $option in
	d)
		digest=1
		...
		if [[ ! -e $currentdigest ]]; then
			...
			lastdigest="/etc/safepac/digest/$(date +%F --date='-1 day').digest"
			if [[ -e $lastdigest ]]; then
				if ((mail)); then
					printf " Sending yesterdays digest... "
					if ! /usr/bin/mail -s "safepac digest for $HOSTNAME on $(date +%F --date='-1 day')" "$mailaddress" < "$lastdigest" ; then
				...
				printf " No digest found for yesterday\n"
			fi
